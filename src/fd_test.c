  ...
  int    nz ) 
{
  ...
  snprintf(current_snap, sizeof(current_snap), "txx_%dx%d_tid_%d.bin", nx, nz, time_step);
  ...
  float     dt )
{
  ...
  for (int index = 0; index < nx*nz; index++)
  {
      ...
      {
        float dtxx_dx =  (FDM8E1*(fld->txx[i + (j-4)*nz] - fld->txx[i + (j+3)*nz]) +
        ...
        float dtxz_dz =  (FDM8E1*(fld->txz[(i-3) + j*nz] - fld->txz[(i+4) + j*nz]) +
      ...
      if((i > 4) && (i < nz-4) && (j >= 4) && (j < nx-4)) 
      {
        float dtxz_dx = (FDM8E1*(fld->txz[i + (j-3)*nz] - fld->txz[i + (j+4)*nz]) +
        ...
        float dtzz_dz = (FDM8E1*(fld->tzz[(i-4) + j*nz] - fld->tzz[(i+3) + j*nz]) +
